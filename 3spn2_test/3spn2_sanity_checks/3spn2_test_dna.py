...
for line in f:
    ...
    angle.createAngle(state.atoms[int(angleInfo[1])], state.atoms[int(angleInfo[2])], state.atoms[int(angleInfo[3])], float(angleInfo[6])/kcal, float(angleInfo[5])*rad)
...
for line in f:
    ...
    if int(diheInfo[5]) == 2:
        ...
        coef = [2.0/kcal,0.0]
        dihePeri.createDihedral(state.atoms[int(diheInfo[1])],state.atoms[int(diheInfo[2])],state.atoms[int(diheInfo[3])],state.atoms[int(diheInfo[4])],coef,math.pi-float(diheInfo[6])*rad)
    else:
