...
AngleBaseStacking::AngleBaseStacking(Atom *a, Atom *b, Atom *c, double theta0_, double epsi_, double sigma_, int type_) {
    ids[0] = a->id;
    ids[1] = b->id;
    ids[2] = c->id;
    theta0 = theta0_;
...
AngleBaseStacking::AngleBaseStacking(double theta0_, double epsi_, double sigma_, int type_) {
    for (int i=0; i<3; i++) {
        ids[i] = -1;
    }
    theta0 = theta0_;
...
std::string AngleBaseStackingType::getInfoString() {
  std::stringstream ss;
  ss << "' theta0='" << theta0 << "' epsi='" << epsi << "' sigma='" << sigma;
  return ss.str();
}

std::string AngleBaseStacking::getInfoString() {
  std::stringstream ss;
  ss << "<member type='" << type << "' theta0='" << theta0 << "' epsi='" << epsi << "' sigma='" << sigma << "' atomID_a='" << ids[0] << "' atomID_b='" << ids[1] << "' atomID_c\
='" << ids[2] << "'/>\n";
  return ss.str();
}

bool AngleBaseStackingType::operator==(const AngleBaseStackingType &other) const {
    return theta0 == other.theta0 and epsi == other.epsi and sigma == other.sigma;
}
