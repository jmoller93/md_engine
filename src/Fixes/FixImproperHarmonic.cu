...
void FixImproperHarmonic::singlePointEng(float *perParticleEng) {
    ...
    //compute_energy_improper<<<NBLOCK(nAtoms), PERBLOCK, sharedMemSizeForParams>>>(nAtoms, state->gpd.xs(activeIdx), perParticleEng, state->gpd.idToIdxs.d_data.data(), forcersGPU.data(), forcerIdxs.data(), state->boundsGPU, parameters.data(), parameters.size(), usingSharedMemForParams, evaluator);
