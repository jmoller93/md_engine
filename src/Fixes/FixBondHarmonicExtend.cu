...
void FixBondHarmonicExtend::createBond(Atom *a, Atom *b, double k, double r0, int type) {
    vector<Atom *> atoms = {a, b};
    validAtoms(atoms);
    if (type == -1) {
        assert(k!=-1 and r0!=-1);
    }
    bonds.push_back(BondHarmonicExtend(a, b, k, r0, type));
    pyListInterface.updateAppendedMember();
    
}

void FixBondHarmonicExtend::setBondTypeCoefs(int type, double k, double r0) {
    assert(r0>=0);
    BondHarmonicExtend dummy(k, r0, type);
    setBondType(type, dummy);
}

void FixBondHarmonicExtend::compute(bool computeVirials) {
